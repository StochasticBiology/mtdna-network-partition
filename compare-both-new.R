...
# default options
#args = c("output-full.csv", 0, 100, 50, "test")
...
vhpred = function(alpha, beta, p, q, h, n) {
  ...
  wc = (1-p)*(1-h)*n
  mc = (1-q)*h*n
  cw2m = wn*mn*(vu + (wn-1)*(eu3 - eu*eu2)) + wc*wn*mn*vu
  cwm2 = wn*mn*(vu + (mn-1)*(eu3 - eu*eu2)) + mc*wn*mn*vu
  cw3m = wn*mn*(vu + 3*(wn-1)*(eu3 - eu*eu2) + (wn^2 - 3*wn + 2)*(eu4 - eu*eu3)) +
    (3/2)*wc*wn*mn*(vu + (wn-1)*(eu3 - eu*eu2)) +
    3*(wc/4 + wc^2 / 4)*wn*mn*vu
  cwm3 = wn*mn*(vu + 3*(mn-1)*(eu3 - eu*eu2) + (mn^2 - 3*mn + 2)*(eu4 - eu*eu3)) +
    (3/2)*mc*wn*mn*(vu + (mn-1)*(eu3 - eu*eu2)) +
    3*(mc/4 + mc^2 / 4)*wn*mn*vu
  cw2m2 = wn*mn*(vu + (wn+mn-2)*(eu3 - eu*eu2) + (wn-1)*(mn-1)*(eu4 - eu2^2)) +
     wc*wn*mn*(vu + (mn-1)*(eu3 - eu*eu2)) +
     mc*wn*mn*(vu + (wn-1)*(eu3 - eu*eu2)) +
     wc*mc*wn*mn*vu
  ...
  mn = mw+mm
  vn = eu*(1-eu)*n + (p*(1-h)+q*h)*n*((p*(1-h)+q*h)*n - 1)*vu
  ...
  return(c(v1, v1+v2, vn, mw,vw,mm,vm,cwm,cw2m,cw3m,cwm2,cwm3,cw2m2,mh,v1,0,0,eu*mass,vu*mass*mass, mn, vn))
}
...
vhpredrepel = function(alpha, beta, p, q, h, n) {
  ...
  # return a long vector with different statistics, mainly ordered to match ordering of the simulation output
  # return zeros for statistics not addressed here
  return(c(v1, 0, vn, mw,vw,mm,vm,cwm,0,0,0,0,0,mh,v1,0,0,eu*mass,vu*mass*mass, mn, vn))
}
...
for(i in 1:nrow(test.df)) {
  ...
  if(repel == T) {
    ...
    pred = vhpred(alpha, beta, test.df$p[i], test.df$q[i], test.df$h[i], n)
  }
  # store comparison in results dataframe
  res.1.df = rbind(res.1.df, data.frame(seeds=test.df$seeds[i], p=test.df$p[i], q=test.df$q[i], h=test.df$h[i], vhsim = test.df$vh[i], vhpred1 = pred[1], vhpred2 = pred[2], vnsim=test.df$vn[i], vnpred=pred[3]))
}
...
plot.h = 0.5
plot.df = res.1.df[res.1.df$h==plot.h,]
...
stats = colnames(test.df[,7:ncol(test.df)])
if(repel == F) {
...
for(i in 1:nrow(test.df)) {
  ...
  for(j in 1:length(stats)) {
    ...
    mom.df = rbind(mom.df, data.frame(seeds=test.df$seeds[i], p=test.df$p[i], q=test.df$q[i], h=test.df$h[i], stat = stats[j], predval = pred[3+j], simval = test.df[i,6+j]))
    }
  }
}
...
png(filename, width=1000*res.factor, height=1000*res.factor, res=72*res.factor)
ggplot(mom.df, aes(x=predval,y=simval)) + geom_point() + geom_abline(slope=1, intercept=0) + facet_wrap(~stat, scales="free")
dev.off()
  ...
  for(n in c(20, 50, 100)) {
    for(p in (0:5)/5) {
	  ...
	  # simulate random variables
          ...
          wc = rbinom(nsamp, wc0, 1/2)
          mc = rbinom(nsamp, mc0, 1/2)
	  ...
	  ndna = w+m
          ...
          pred = vhpred(alpha, beta, p, q, h, n)
