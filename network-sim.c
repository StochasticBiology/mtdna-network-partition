...
void GetStats(double *xs, double *ys, double *xe, double *ye, int n, double *mx, double *my, int *mt, int *networked, int *wn, int *wc, int *mn, int *mc, double *d, double *u)
{
  int i, j;
  int counter;
  double mindist;
  ...
  // hstat -- this simulation's heteroplasmy for y > 0
  // nstat -- this simulation's mtDNA count for y > 0
  *wc = *wn = *mc = *mn = 0;
  for(i = 0; i < MAXM; i++)
    {
      if(my[i] > 0)
  ...
  for(i = 0; i < n; i++)
    {
      ...
      if(ye[i] > 0 && ys[i] > 0) propupper = 1;
      else if(ye[i] < 0 && ys[i] < 0) propupper = 0;
      else if(ye[i] > ys[i]) propupper = ye[i]/(ye[i]-ys[i]);
      else propupper = ys[i]/(ys[i]-ye[i]);
...
int main(int argc, char *argv[])
{
  ...
  // file for output
  sprintf(str, "output-%.3f-%.0f.csv", het, nseed);
  fpout = fopen(str, "w");
  fprintf(fpout, "h,seeds,p,q,lambda,halo,");
	      ...
	      for(halo = halos; halo <= haloe; halo += haloa)
		{
		  ...
		  for(sim = 0; sim < NSIM; sim++)
			...
			Output(xs, ys, xe, ye, n, mx, my, mt, het, nseed, inc0, inc1, lambda, halo, perturbtype);
		      ...
		      GetStats(xs, ys, xe, ye, n, mx, my, mt, networked, &wn, &wc, &mn, &mc, &d, &u);
		      stats[sim].wn = wn;
		      stats[sim].wc = wc;
		      stats[sim].mn = mn;
		      stats[sim].mc = mc;			      
		  ...
		  fprintf(fpout, "%.2f,%.0f,%f,%f,%f,%f,", het, nseed, inc0, inc1, lambda, halo);
